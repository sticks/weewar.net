...
namespace Eliza
{
  public class ElizaApi
  {
    ...
    {
      String s = DoRequest(uri, user, password, xml);
    ...
    private String DoRequest(String u, String username, String password, String xml)
    {
      ...
      if (xml != null)
      {
        ...
        writer.Write(xml);
        File.AppendAllText("c:/temp/eliza.log", xml);
        writer.Close();
      }
      try
      {
        ...
        File.AppendAllText("c:/temp/eliza.log", _lastResult);
        return _lastResult;
      }
      catch (WebException )
      {
        // just let the outer handle it
        throw;
      }
    }
    ...
    public WeewarMap GetMap(int mapId)
    {
      ...
      XmlDocument doc = new XmlDocument();
      doc.Load(reader);
      return new WeewarMap(doc.DocumentElement);
    }
